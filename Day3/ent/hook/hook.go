...
// The DevelopperFunc type is an adapter to allow the use of ordinary
// function as Developper mutator.
type DevelopperFunc func(context.Context, *ent.DevelopperMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DevelopperFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.DevelopperMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DevelopperMutation", m)
	}
	return f(ctx, mv)
}

...
func (cq *ContactQuery) QueryOwner() *DevelopperQuery {
...
func (cq *ContactQuery) WithOwner(opts ...func(*DevelopperQuery)) *ContactQuery {
	query := &DevelopperQuery{config: cq.config}
...
func (cq *ContactQuery) sqlAll(ctx context.Context) ([]*Contact, error) {
	...
	if query := cq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].developper_contact; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "developper_contact" returned %v`, n.ID)
			}
			for i := range nodes {
